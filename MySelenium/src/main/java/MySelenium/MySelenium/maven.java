...
public static void main(String[] args) {
  ...
  WebElement  laptops= driver.findElement(By.xpath("//*[@id='menu-electronics-tab-0-content']/ul[1]/li[1]/a"));
  mobiles.click();//clicking on laptops
  ...
  for(int i=0;i<brands.size();i++){
   ...
   if(brands.get(i).getText().equals("Dell G3 core i7 10th gen"))//matching for dell only
   {
    brands.get(i).click();//clicking on laptops
   }
  ...
  for(int i=0;i<data.size();i++){
   System.out.println(data.get(i).getText());//printing all laptops
   if(data.get(i).getText().contains("Dell G3"))//search for g3 only
   {
